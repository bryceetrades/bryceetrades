...
function subscribeToContract(contract_id) {
  sendSubscription({ proposal_open_contract: 1, contract_id }, (data) => {
    ...
    if (poc.is_sold) {
      ...
      const settledDigit =
        exitPrice != null
          ? Number(exitPrice.toString().slice(-1))
          : tickWindow.length
          ? tickWindow[tickWindow.length - 1].digit
          : null;
...
function showTradeToast(stake, profit) {
  ...
  toast.innerHTML = ` ${isWin ? "+" : ""}${profit.toFixed(2)} USD <span class="stake-line">Stake: ${stake.toFixed(2)} USD</span> `;
